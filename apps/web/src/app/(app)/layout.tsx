...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  const { status, user, logout, viewingAs } = useAuth();
   ...
   *
   * Not while viewing somebody else's account. The flag belongs to the member
   * being looked at, and staff who open the account of a student they have just
   * issued a temporary password to would otherwise be dropped onto that
   * student's "choose your own password" screen — which is both useless and an
   * invitation to set a password on their behalf. The borrowed token carries no
   * such claim, so the API never asked for this redirect.
   */
  useEffect(() => {
    if (status === 'authenticated' && user?.mustChangePassword && !viewingAs) {
      router.replace('/change-password');
    }
  }, [status, user?.mustChangePassword, viewingAs, router]);

  useEffect(() => {
    setDrawerOpen(false);
  }, [pathname]);

  // Same exception as the redirect above: the flag belongs to the member being
  // looked at, not to the staff member looking.
  if (status === 'authenticated' && user?.mustChangePassword && !viewingAs) {
